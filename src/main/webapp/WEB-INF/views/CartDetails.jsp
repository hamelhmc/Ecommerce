		...
		<table class="table table-hover">
			...
			<tbody>
				<form:form method="POST" modelAttribute="userProfile"
					...
					<c:forEach items="${userProfile.cartline}" var="cartline"
						varStatus="varStatus">
						<tr>
							<td><spring:url
								...
								<form:hidden
									path="cartline[${varStatus.index}].product.productId" /></td>
							<td><fmt:formatNumber value="${cartline.product.price}"
									type="currency" /></td>

							<td><form:input path="cartline[${varStatus.index}].quantity"
									type="number" /></td>
							<td><fmt:formatNumber
									value="${cartline.quantity*cartline.product.price}"
									type="currency" /></td>
